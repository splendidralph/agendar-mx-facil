-- Fix profile picture upload issue by making validation smarter
-- The issue: Profile image uploads trigger full profile validation including colonia requirement
-- Solution: Make validation context-aware - skip colonia validation for profile image only updates

DROP FUNCTION IF EXISTS public.validate_provider_profile() CASCADE;
